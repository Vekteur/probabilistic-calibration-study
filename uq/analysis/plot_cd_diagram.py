# Code adapted from https://github.com/hfawaz/cd-diagram/blob/master/main.py
...
def graph_ranks(
    ...
    **kwargs,
    ...
    cliques = [(min(clq), max(clq)) for clq in cliques]

    def in_set(itv1, itv2):
        min1, max1 = itv1
        min2, max2 = itv2
    ...
    while i < len(cliques):
        j = 0
        while j < len(cliques):
            if i != j and in_set(cliques[i], cliques[j]):
                cliques.pop(i)
                break
            j += 1
        else:
    ...
    # cliques = list(dict.fromkeys(cliques))
...
def wilcoxon_holm(alpha=0.05, df_perf=None):
    ...
    # WARNING: the number of datasets has to be the same for all models
    classifiers = list(df_counts.loc[df_counts['count'] == max_nb_datasets]['model'])

    # test the null hypothesis using friedman before doing a post-hoc analysis
    friedman_p_value = friedmanchisquare(
        *(np.array(df_perf.loc[df_perf['model'] == c]['value']) for c in classifiers)
    )[1]
    if friedman_p_value >= alpha:
        # then the null hypothesis over the entire classifiers cannot be rejected
        raise ValueError('the null hypothesis over the entire classifiers cannot be rejected')
    ...
    for i in range(m - 1):
        ...
        perf_1 = np.array(
            df_perf.loc[df_perf['model'] == classifier_1]['value'],
            dtype=np.float64,
        )
        for j in range(i + 1, m):
            ...
            # get the performance of classifier one
            perf_2 = np.array(
                df_perf.loc[df_perf['model'] == classifier_2]['value'],
                dtype=np.float64,
            )
    ...
    sorted_df_perf = df_perf.loc[df_perf['model'].isin(classifiers)].sort_values(['model', 'dataset'])
    # get the rank data
...
def draw_my_cd_diagram(df, metric):
    if metric == 'test_nll':
        df = df.query('pred_type != "quantile"')

    df = df[metric].reset_index(level='run_id', drop=True)
    df = df.groupby(df.index.names, dropna=False).mean().reset_index()

    df = df[['dataset', 'name', metric]]
    df = df.rename(columns={metric: 'value', 'name': 'model'})
    df['value'] = df['value'].astype(float)

    # Remove rows with nan values, but in that case give a warning
    ...
    if metric == 'test_nll':
        values = df.query('model == "MIX-NLL + Recal"')['value'].unique()
        #print(values, type(values))
    kept_models = df.query('not value.isna()')['model'].unique()
    df = df.query('model in @kept_models')

    df['value'] = -df['value']
    return draw_cd_diagram(df)
